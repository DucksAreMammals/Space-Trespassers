...
func _process(delta):
	...
	if Input.is_action_pressed("ui_accept") and OS.get_ticks_usec() > time_of_next_bullet:
		time_of_next_bullet = OS.get_ticks_usec() + time_between_bullets * 1000000
...
func _resume():
	time_of_next_bullet += OS.get_ticks_usec() - pause_time

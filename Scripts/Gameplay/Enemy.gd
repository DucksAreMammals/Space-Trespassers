...
func _process(_delta):
	# ((microsecods to seconds) add 9999 to desync the ships) times ((360 degrees in radians) divided by wave speed) times wave amount
	# multiplies seconds by two pi over wave speed. This makes sure that the
	# wave speed is seconds per cycle. Then multiply the result of sin by wave
	# amount so that the wave is more exaggerated.
	# Commented because I should use animation
